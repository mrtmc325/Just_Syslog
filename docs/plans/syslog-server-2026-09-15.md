...
Updated 2026-09-16: added a macOS menu bar controller app (`packaging/macos/menubar-app/`, Swift/AppKit, no third-party deps) bundled into the `.pkg` at `/Applications/Syslog Collector.app`. Reads live status from the loopback HTTP API (no privilege); start/stop/clear/config-save act on the root launchd daemon via `launchctl` behind a macOS admin prompt. No Rust changes. Built + launch-verified (arm64) here.
Updated 2026-09-16: built the Linux `.deb` + `.rpm` (amd64) for real via a Docker cross-build (`packaging/linux/build-in-docker.sh`) — no host Rust/nfpm needed. Switched nfpm templating from `${VAR}` env-expansion (unreliable) to sed-rendered `@@VAR@@` placeholders. Both packages contents-verified (files, metadata, systemd unit, postinstall/preremove scriptlets).
Updated 2026-09-16: fixed the macOS pkg not surfacing the menu bar app. Root cause: pkgbuild defaulted the `.app` to **relocatable**, so the installer could place it anywhere (or nowhere the user looked), and nothing launched it post-install. Fix: `--component-plist` with `BundleIsRelocatable=false` (always `/Applications`) + a login LaunchAgent (`packaging/launchd/…menubar.plist`) plus a postinstall that starts it in the console user's GUI session immediately. Auto-launch also avoids the Gatekeeper double-click prompt for the ad-hoc-signed app.
Updated 2026-09-16: gave the menu bar app an icon (was showing the generic app icon in Finder/Launchpad). `make-icon.swift` renders a broadcast glyph on a slate squircle; `build-app.sh` turns it into `AppIcon.icns` (sips/iconutil) before signing, referenced via `CFBundleIconFile`.
